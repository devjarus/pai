...
### Fixed
- **Quiet watch evaluations no longer resurface stale digests** — When a watch/program is re-evaluated without producing a new digest, the API now stops promoting the previous digest summary as the current "latest" one. This prevents repeated "No significant updates since …" research reports from reappearing as fresh watch activity until a genuinely new digest is delivered.
- **Leaked `<tool_call>` markup no longer shows in Digests** — Research reports that dumped raw model tool-call XML (`<tool_call>`, `<arg_key>`, `<arg_value>`) into Digests are stripped on ingest and in Digests rendering. Tool-call-only output now triggers the synthesis pass, and empty fallback `renderSpec` shells no longer hide the “No report available” state.
