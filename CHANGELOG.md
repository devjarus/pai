...
- **Graphical analysis artifact visibility** — Telegram now forwards artifacts generated by `run_code` (for example charts and `analysis.md`) in chat replies, and the web chat `run_code` tool card now renders stdout/stderr plus downloadable artifact links so analysis outputs are visible in both clients.
