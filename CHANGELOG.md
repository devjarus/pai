...
### Fixed
- **Daily digest duplicate suppression for failed/unchanged watch cycles** — Background (scheduled/maintenance) daily digest runs now skip publishing a new ready digest when the computed signal hash matches the last delivered daily digest (the duplicate attempt is marked `skipped`). This prevents repeated “keep this watch” cards from appearing over and over when upstream watch jobs fail or no material signal changed.
