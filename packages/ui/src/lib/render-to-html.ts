/**
 * Convert a json-render spec to static HTML with inline CSS for PDF export.
 * Browser-only: uses `window.location.origin` for relative URL resolution
 * and `marked` for Markdown rendering.
 */
...
/** Resolve relative URLs to absolute so they work in the blank print window */
...
function renderElement(id: string, spec: JsonRenderSpec): string {
  const el = spec.elements[id];
  if (!el) return "";
  const props = (el.props ?? {}) as Record<string, unknown>;
  const childrenHtml = (el.children ?? []).map((cid) => renderElement(cid, spec)).join("\n");
  ...
  switch (el.type) {
    ...
    case "Markdown": {
      ...
      return marked.parse(content, { gfm: true, breaks: false, async: false }) as string;
    }
