...
export function GridCard({ card, pinned, unseen, onTogglePin, onOpen }: GridCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Title row: title + (badge + timestamp if no subtitle) */}
      <div className="flex items-start justify-between gap-2 mb-1">
        <h3 className="text-sm font-semibold text-foreground line-clamp-2">{card.title}</h3>
        <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
          <span className={cn("inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider", cfg.badge)}>
      ...
      {/* Subtitle + timestamp — only when subtitle exists */}
      {card.subtitle && (
